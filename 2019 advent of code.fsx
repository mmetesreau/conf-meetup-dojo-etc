...
module MetesreauDay2 = 
    ...
    let read (memory: int array) (instructionPointer: int) =
        ...
        | 1 -> (memory.[instructionPointer + 1], 
                memory.[instructionPointer + 2], 
                memory.[instructionPointer + 3]) 
                    |> Add
        | 2 -> (memory.[instructionPointer + 1], 
                memory.[instructionPointer + 2], 
                memory.[instructionPointer + 3]) 
                    |> Multiply 
        | 99 -> Halt
    ...
    let execute (memory: int array) =
        let rec execute' (memory: int array) (instructionPointer: int) =
            match read memory instructionPointer with
            | Halt -> memory
            | instruction ->
                ...
                let offset = 4
                instructionPointer + offset |> execute' memory 
    ...
    seq {
         for noun in [0..99] do 
            for verb in [0..99] -> (100 * noun + verb), input 
                                                            |> load
                                                            |> init noun verb
                                                            |> execute
                                                            |> dumpOutput                          
...
module MetesreauDay3 = 
    ...
    intersections
        |> Seq.map (fun intersection -> positionsLookup |> Seq.map (fun path -> path |> Map.find intersection) |> Seq.sum)
        |> Seq.sort
        |> Seq.skip 1
        |> Seq.take 1

module MetesreauDay4 = 
...
module MetesreauDay5 = 
    ...
    let getParameter (memory: int array) (mode: Modes) (parameterPointer: int) =
        let parameter = memory.[parameterPointer]
        match mode with
        | Position -> memory.[parameter]
        | _ -> parameter

    let parseInstruction (entry: int) =
    ...
    let (|AddInstruction|_|) (parameterModes, opcode) =
        if opcode = 1 then
             (parameterModes |> Seq.tryItem 0 |> parseMode, 
              parameterModes |> Seq.tryItem 1 |> parseMode, 
              Immediate) |> Some
        else None     

    let (|MultiplyInstruction|_|) (parameterModes, opcode) =
        if opcode = 2 then
             (parameterModes |> Seq.tryItem 0 |> parseMode, 
              parameterModes |> Seq.tryItem 1 |> parseMode, 
              Immediate) |> Some
    ...
    let (|PrintInstruction|_|) (parameterModes, opcode) =
        if opcode = 4 then
            parameterModes |> Seq.tryItem 0 |> parseMode |> Some
        else None 

    let (|JumpIfTrueInstruction|_|) (parameterModes, opcode) =
        if opcode = 5 then
             (parameterModes |> Seq.tryItem 0 |> parseMode, 
              parameterModes |> Seq.tryItem 1 |> parseMode) |> Some
        else None 

    let (|JumpIfFalseInstruction|_|) (parameterModes, opcode) =
        if opcode = 6 then
             (parameterModes |> Seq.tryItem 0 |> parseMode, 
              parameterModes |> Seq.tryItem 1 |> parseMode) |> Some
        else None 

    let (|LessThanInstruction|_|) (parameterModes, opcode) =
        if opcode = 7 then
             (parameterModes |> Seq.tryItem 0 |> parseMode, 
              parameterModes |> Seq.tryItem 1 |> parseMode, 
              Immediate) |> Some
    ...
    let (|EqualsInstruction|_|) (parameterModes, opcode) =
        if opcode = 8 then
            (parameterModes |> Seq.tryItem 0 |> parseMode, 
             parameterModes |> Seq.tryItem 1 |> parseMode, 
             Immediate) |> Some
    ...
    let read (memory: int array)  (instructionPointer: int) =
        match memory.[instructionPointer] |> parseInstruction  with
        | AddInstruction (pm1, pm2, pm3) ->
            (getParameter memory pm1 (instructionPointer+1), 
             getParameter memory pm2 (instructionPointer+2), 
             getParameter memory pm3 (instructionPointer+3)) 
                |> Add
    ...
    let execute (readInput: unit -> string) (memory: int array) =
        let rec execute' (memory: int array) (instructionPointer: int) =
            match read memory instructionPointer with
            | Halt -> memory
            | instruction ->
                let instructionPointer' =
                    ...
                    | Add (i1, i2, dest) -> 
                        memory.[dest] <- i1 + i2
                        instructionPointer + getOffset instruction
                    | Multiply (i1, i2, dest) -> 
                        memory.[dest] <- i1 * i2
                        instructionPointer + getOffset instruction
                    | Print i1 -> 
                        ...
                        instructionPointer + getOffset instruction
                    | Write i1 -> 
                        memory.[i1] <- readInput() |> int
                        instructionPointer + getOffset instruction
                    | JumpIfTrue (i1, i2) -> 
                        if i1 = 0 then instructionPointer + getOffset instruction
                        else i2 
                    | JumpIfFalse (i1, i2) -> 
                        if i1 = 0 then i2
                        else instructionPointer + getOffset instruction
                    | LessThan  (i1, i2, dest) -> 
                ...
                instructionPointer' |> execute' memory
    ...
    let createReadInputsFrom (buffer: string list) =
    ...
    input    
        |> load 
        |> execute (createReadInputsFrom ["1"])

    // --- Part Two ---

    input    
        |> load 
        |> execute (createReadInputsFrom ["5"])

module MetesreauDay6 =
    [<AutoOpen>]
    module Helpers = 
        let split (separator: char) (str: string) = str.Split(separator)
...
module MetesreauDay7 = 
    ...
    let rec execute (readInput: unit -> string) (memory: int array) (instructionPointer: int) =
        match read memory instructionPointer with
        | Halt -> None
        | instruction ->
            let output, nextInstructionPointer =
                match instruction with
                | Add (i1, i2, dest) -> 
                    memory.[dest] <- i1 + i2
                    None, instructionPointer + getOffset instruction
                | Multiply (i1, i2, dest) -> 
                    memory.[dest] <- i1 * i2
                    None, instructionPointer + getOffset instruction
                | Print i1 -> 
                    Some (string i1), instructionPointer + getOffset instruction
                | Write i1 -> 
                    memory.[i1] <- readInput() |> int
                    None, instructionPointer + getOffset instruction
                | JumpIfTrue (i1, i2) -> 
                    if i1 = 0 then None, instructionPointer + getOffset instruction
                    else None, i2 
                | JumpIfFalse (i1, i2) -> 
                    if i1 = 0 then None, i2
                    else None, instructionPointer + getOffset instruction
                | LessThan  (i1, i2, dest) -> 
                    memory.[dest] <- if i1 < i2 then 1 else 0
                    None, instructionPointer + getOffset instruction
                | Equals (i1, i2, dest) -> 
            ...
            match output with
            | Some output -> Some (output, memory, nextInstructionPointer)
            | None -> nextInstructionPointer |> execute readInput memory
    ...
    let executeUntilHalted (readInput: unit -> string) (memory: int array) =
        let rec executeUntilHalt' (output: string) (readInput: unit -> string) (memory: int array) (instructionPointer: int) =
            match instructionPointer |> execute readInput memory with
            | None -> 
                output
            | Some (output', memory', instructionPointer') -> 
                executeUntilHalt' (sprintf "%s%s" output output') readInput memory' instructionPointer'

        executeUntilHalt' "" readInput memory  0

    let connectInSerie (program: string) =
        List.fold (fun signal setting ->  
                                program 
                                    |> load 
                                    |> executeUntilHalted (createReadInputsFrom [setting;signal])) "0"
        
    let feedbackLoop (programs: (int [] * int) list) (settings: string list) =
        let rec feedbackLoop' (programs: (int array * int) list) (signal: string) (settings: string list) =
            let readInput, settings' = 
                match settings with
                | setting::settings -> 
                    createReadInputsFrom [setting;signal], settings
                | [] -> 
                    createReadInputsFrom [signal], []   
        
            match programs with
            | [] -> signal
            | (memory, instructionPointer)::programs ->
                match instructionPointer |> execute readInput memory with
                | Some (signal', memory', instructionPointer') ->
                    feedbackLoop' (programs @ [memory', instructionPointer']) signal' settings'
                | None -> 
