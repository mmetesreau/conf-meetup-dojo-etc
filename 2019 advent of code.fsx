module MetesreauIntcodeComputer =
    ...
    let rec executeInstruction (input: unit -> string) (memory: Map<int64, int64>) (relativeBase: int64) (instructionPointer: int64) =
        ...
        | instruction ->
            ...
            | Write dest ->
                let i = input()
                printfn "%A" i
                let memory' = i |> int64 |> writeToMemory memory dest
                "", memory', relativeBase, instructionPointer + getInstructionOffset instruction
            | JumpIfTrue (i1, i2) -> 
...
module MetesreauDay7 = 
    ...
    let executeProgramUntilFeedback (input: unit -> string) (memory: Map<int64, int64>) (relativeBase: int64) (instructionPointer: int64) =
        let rec executeProgramUntilFeedback' (input: unit -> string) (memory: Map<int64, int64>) (relativeBase: int64) (instructionPointer: int64) =
            match instructionPointer |> executeInstruction input memory relativeBase with
            | None -> 
                None
            | Some (output', memory', relativeBase', instructionPointer') -> 
                if output'.Length = 0 then
                    executeProgramUntilFeedback' input memory' relativeBase' instructionPointer'
                else
                    Some (output', memory', relativeBase', instructionPointer')

        executeProgramUntilFeedback' input memory 0L 0L 

    let feedbackLoop (programs: (Map<int64,int64> * int64) list) (settings: string list) =
        let rec feedbackLoop' (programs: (Map<int64,int64> * int64) list) (signal: string) (settings: string list) =
            let input, settings' = 
                match settings with
                | setting::settings -> 
            ...
            | (memory, instructionPointer)::programs ->
                match instructionPointer |> executeProgramUntilFeedback input memory 0L with
                | Some (signal', memory', _, instructionPointer') ->
